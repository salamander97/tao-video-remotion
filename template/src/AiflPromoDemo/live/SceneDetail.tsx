      ...
      {rows.map((r, i) => {
        ...
        if (df >= cue && df < cue + 16) {
          ...
          flyer = (
            ...
            >
              {/* crop-window: full texture positioned so the (r.x, r.y) slice
                  lands at the wrapper's origin; wrapper's overflow:hidden clips
                  the rest — Remotion-safe replacement for background-image
                  cropping (same visual, `<Img>` instead of CSS background). */}
              <Img
                src={detailSrc}
                style={{ position: 'absolute', left: -r.x, top: -r.y, width: 1920, height: DETAIL_H }}
                alt=""
              />
            </div>
          );
