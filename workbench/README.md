...
方法论与成片接入契约见 [`vendor/shotcraft/workbench.md`](../vendor/shotcraft/workbench.md)。
...
- **素材库四 tab**：素材（成片单元 + 工程 public/ 里的图片视频）· 动效库（demos/ 216 个 demo 按画廊
  10 类折叠，有本地样片的循环预览，没样片的卡显示定妆帧、悬停才播放）· 音效（本片音频 + `assets/audio/sfx` 144 个音效按 15 类 +
  4 首 BGM，另有 6 个来源未核实、隔离不收录——数字以 `vendor/shotcraft/audio/manifest.json` 为准）·
  背景（纸底 / 纯白 / 墨黑 / 纸底提亮）。**点击=中屏预览，拖拽到时间轨=添加**
...
工程要提供 `src/workbench.ts` 清单才能拆解导入（写法见 vendor/shotcraft/workbench.md §2，范例
